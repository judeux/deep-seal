...
- 캠페인 귀환, 저장, 봉인석 진행.
- 벽과 지형을 우회하는 적 경로 탐색.
- 시간 또는 최소 적 수 기준의 런타임 적 스폰.
- 적 처치 또는 채굴에 따른 즉시 보상 드랍과 자동 획득.
- 정식 원정 결과 화면과 보상 정산.
- 캠페인 귀환, 저장, 봉인석 진행.

다음 구현 후보:

1. Enemy navigation and spawn pressure pass
   - 벽에 막힌 적이 멈추지 않도록 그리드 기반 경로 탐색을 추가.
   - 적별 이동 속도나 우선순위 차이를 추가해 전투 압박을 단조롭지 않게 만듦.
   - 시간 간격 또는 최소 적 수 유지 방식의 런타임 스폰을 추가.
2. Prototype reward drops
   - 적 처치와 일부 채굴 행동에서 보상 드랍을 생성.
   - 플레이어가 일정 거리 안으로 접근하면 자동 획득되도록 구현.
3. Upgrade selection prototype
   - 적 압박과 보상 루프가 안정된 뒤 첫 강화 선택을 검증.
